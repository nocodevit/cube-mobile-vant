...
**问题描述**：
- `van-search` 组件的清除按钮在移动端（手机）可以正常清除
- 但在桌面端（电脑浏览器）无法清除，点击清除按钮无响应

**现象**：
- 移动端：✅ 清除功能正常
- 桌面端：❌ 清除功能失效
- 即使添加了 `clearable` 属性和 `@clear` 事件监听器，桌面端仍然无法正常工作

**可能原因**：
- 移动端和桌面端的事件处理机制不同（触摸事件 vs 鼠标事件）
- 可能是 Vant 组件库的 bug，但未在官方文档或 GitHub Issues 中找到相关记录

**临时解决方案**：
- 桌面端用户可以通过手动删除输入框内容来清除搜索
- 建议在 Vant 官方 GitHub 提交 issue 报告此问题

**注意**：此问题尚未在 Vant 官方文档中确认为已知缺陷，需要进一步验证。
